...
extrae los rayos a nivel mundial y guarda un archivo GeoJSON por cada
descarga. Mantiene los últimos 80 archivos (≈ 20 días a 4 descargas/día).

URL pública WWLLN: https://wwlln.net/WWLLN.kmz
  → Se actualiza cada hora con 1 hora de datos globales (retraso ~6 h)

Archivos de salida:
  ../wwlln/wwlln_YYYY-MM-DD_HHMM.geojson   → snapshot por descarga
...
# Bounding box: cobertura mundial (todo el planeta).
# Para limitar a México y región, usar: LAT 10/36 y LON -122/-76.
LAT_MIN = -90.0
LAT_MAX =  90.0
LON_MIN = -180.0
LON_MAX =  180.0
REGION_DESC = "Mundial — todo el planeta"
...
def main():
    print("=" * 60)
    print("  WWLLN Updater — CU Tlaquepaque (cobertura mundial)")
    print("=" * 60)
    ...
    print(f"  Procesando rayos ({REGION_DESC})...")
    ...
    print(f"  Rayos           : {len(features)} (cobertura mundial)")
